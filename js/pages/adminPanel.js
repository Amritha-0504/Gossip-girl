...
async function renderRequests() {
  ...
  const userMap = {};
  await Promise.all(userIds.map(uid => getUser(uid).then(u => { userMap[uid] = u; })));
  ...
  el.innerHTML = reqs.map(r => {
    ...
    const statusBadge = r.status === "pending" ? `<span style="color:#fbbf24;font-size:0.72rem;font-weight:600;">⏳ Pending</span>`
      : r.status === "accepted" ? `<span style="color:#4ade80;font-size:0.72rem;font-weight:600;">✅ Accepted</span>`
        : `<span style="color:#f87171;font-size:0.72rem;font-weight:600;">❌ Rejected</span>`;
    return `
      <div class="gg-card" style="margin-bottom:0.75rem;padding:1rem;" data-request-id="${r.requestId}">
        <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:${r.status === "pending" ? "0.75rem" : "0"}">
          <div style="width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff;flex-shrink:0;background:${isEmoji ? "#1e1b4b" : col};font-size:${isEmoji ? "1.2rem" : "0.9rem"}">
            ${escapeHtml(av)}
          </div>
          <div style="flex:1;min-width:0;">
            <p style="color:#fff;font-weight:600;font-size:0.9rem;">${name}</p>
            <p style="color:#9ca3af;font-size:0.75rem;">${escapeHtml(u?.email || u?.userId || "")}</p>
            ${statusBadge}
          </div>
        </div>
        ${r.status === "pending" ? `
        <div style="display:flex;gap:0.5rem;">
          <button class="accept-btn" data-uid="${r.userId}" data-rid="${r.requestId}"
            style="flex:1;padding:0.55rem;border-radius:10px;border:none;font-weight:700;font-size:0.85rem;cursor:pointer;background:linear-gradient(135deg,#22c55e,#16a34a);color:#fff;transition:all 0.2s;">
            ✓ Accept as Author
          </button>
          <button class="reject-btn" data-uid="${r.userId}" data-rid="${r.requestId}"
            style="flex:1;padding:0.55rem;border-radius:10px;border:1px solid rgba(239,68,68,0.4);font-weight:700;font-size:0.85rem;cursor:pointer;background:rgba(239,68,68,0.1);color:#f87171;transition:all 0.2s;">
            ✕ Reject
          </button>
        </div>` : ""}
      </div>`;
...
async function renderUsers() {
  ...
  function renderList(filter = "") {
    ...
    listEl.innerHTML = filtered.map(u => {
      ...
      return `
        <div class="gg-card" style="display:flex;align-items:center;gap:0.75rem;padding:0.85rem;">
          <div style="width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:#fff;flex-shrink:0;background:${isEmoji ? "#1e1b4b" : col};font-size:${isEmoji ? "1.1rem" : "0.85rem"}">
            ${escapeHtml(av)}
          </div>
          <div style="flex:1;min-width:0;">
            <p style="color:#fff;font-size:0.88rem;font-weight:600;">${escapeHtml(publicName(u))}</p>
            ${u.userId ? `<p style="color:#a78bfa;font-size:0.72rem;font-family:monospace;">@${escapeHtml(u.userId)}</p>` : ""}
            <p style="color:#6b7280;font-size:0.72rem;">${escapeHtml(u.email || "")}</p>
          </div>
          <div style="display:flex;align-items:center;gap:0.5rem;flex-shrink:0;">
            <span class="role-badge badge-${u.role || "viewer"}" style="font-size:0.7rem;">${u.role || "viewer"}</span>
            ${isAuthor ? `
            <button class="demote-btn" data-uid="${u.uid}"
              style="padding:0.3rem 0.6rem;border-radius:7px;border:1px solid rgba(239,68,68,0.35);background:rgba(239,68,68,0.08);color:#f87171;font-size:0.72rem;font-weight:600;cursor:pointer;white-space:nowrap;transition:all 0.2s;"
              title="Remove from author role">
              Remove Author
            </button>` : ""}
          </div>
        </div>`;
    }).join("") || `<p style="color:#9ca3af;font-size:0.88rem;text-align:center;padding:1rem;">No users found.</p>`;

    // Demote handlers
    listEl.querySelectorAll(".demote-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        const uid = btn.dataset.uid;
        if (!confirm("Remove this user from author role? They will become a viewer.")) return;
        btn.disabled = true; btn.textContent = "Removing…";
        await setUserRole(uid, "viewer");
        showToast("Author demoted to viewer.", "info");
        await renderUsers();
      });
    });
  }
